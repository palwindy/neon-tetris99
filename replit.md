...
- **5.14** — (1) VS CPU HP初期値0化（キャッシュ問題修正）、HP×2（Lv1=20〜Lv5=100）。(2) CPUダンジャーBGMピッチアップ閾値を固定20→初期HP÷5（Lv1=4以下、Lv5=20以下）に変更。(3) VS CPUクリアタイム計測機能（コンマ2桁）追加：ゲーム中表示・勝利リザルト表示・ニューレコードフラグ・レベルごとの最速ベストタイム保持。(4) VS CPUレベル選択画面に各レベルのベストタイムを小さく表示。(5) SINGLEモードにスコア記録機能追加（スコア・Lv・ライン数をTOP5保持、ゲームオーバー時にランクとTOP5表示）。
